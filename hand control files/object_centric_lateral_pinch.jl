...
function object_centric_lateral_pinch(box_width, box_thickness)
    ...
    for i in 1:length(attracted_frames)
        ...
        add_component!(vm_robot, PointMass(1.0, "$(attracted_frames_names[i]) ee position"); id="$(attracted_frames_names[i]) ee mass")
    ...
    with_rospy_connection(Sockets.localhost, ROSPY_LISTEN_PORT, 24, 48) do connection
        ros_vm_position_controller(connection, cvms, qᵛ, joint_names; f_control, f_setup, E_max=50.0)
    end
