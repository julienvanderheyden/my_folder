...
function ros_vm_position_controller(
        connection::ROSPyClientConnection,
        vms,
        qᵛ,
        joint_names; 
    ...
    control_func! = let control_cache=control_cache, args=args
        function control_func!(torques, state, i, t, dt)
            ...
            if t > 10.0
                @info "10 seconds reached, stopping controller"
                return true
            end
            return false
        end
...
# only the virtual mechanism

# function ros_vm_motion_generator(
#         connection::ROSPyClientConnection,
#         vms,
#         qᵛ; 
#         gravity=DEFAULT_GRAVITY,
...
#             # Main control step
#             f_control(control_cache, t, args, (dt, i)) # Call user control function
#             dynamics_step!(control_cache, t)
#             torques .= get_q(control_cache) # We can change this to whatever we would like to send to the robot
#             return false
#         end
#     end
#     # Check that stored energy is within bounds
